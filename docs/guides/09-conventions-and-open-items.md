...
| 6 | Webhook signing scheme (HMAC? which header carries the signature?) | `guides/07-webhooks.md` | Needed to verify a webhook is really from this platform, not spoofed |
| 7 | Whether `webhooks[].secret` is client-supplied or server-generated | `guides/07-webhooks.md` | Changes both the request contract and the security posture |
